...
graph TD
  ...
  Router --> CustomChart["CustomChartPage.vue"]
...
| カスタムチャートリンク | 認証済み | `/custom-chart` へ遷移 |
...
カスタムチャートページ。API から取得したデータソースをユーザーが選択し、Lightweight Charts で描画する。

- マウント時に `getCustomChartSources()` でソース一覧・軸グループ定義を取得
- ユーザーがチェックボックスでソースを選択し「反映」ボタンで描画
- 複数系列の折れ線チャート、Y軸は最大 2 軸（右・左）
- 軸グループ（`axis_group`）とローカルグループ（`local_group`）によるソース分類表示
- ウィンドウリサイズ対応
- 依存: `getCustomChartSources()` API, `getCustomChartData()` API, Lightweight Charts, PrimeVue
...
| CustomChartPage | - | - | getCustomChartSources, getCustomChartData | Lightweight Charts, PrimeVue |
